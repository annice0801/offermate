    ...
    <div className="space-y-6 text-[17px] leading-[1.9] text-ink/85">
      {blocks.map((b, i) => {
        switch (b.type) {
          ...
          case "image":
            // 內文配圖：左右稍微超出文字欄寬，形成節奏感；下方附圖說（授權標示集中在文末「本文圖片來源」）
            return (
              <figure key={i} className="py-4 sm:-mx-6">
                ...
                {b.caption && <figcaption className="mt-3 px-1 text-center text-sm text-ink/60 sm:px-6">{b.caption}</figcaption>}
              </figure>
            );
            ...
            return (
              <div key={i} className="grid gap-3 text-[15px] leading-relaxed sm:grid-cols-2">
                <div className="rounded-2xl bg-bad-soft/80 p-5">
                  <p className="mb-2 flex items-center gap-1 text-xs font-semibold tracking-wide text-bad">
                    <X className="size-3.5" strokeWidth={3} /> 常見說法
                  </p>
                  <p>「{b.bad}」</p>
                </div>
                <div className="rounded-2xl bg-good-soft/80 p-5">
                  <p className="mb-2 flex items-center gap-1 text-xs font-semibold tracking-wide text-good">
                    <Check className="size-3.5" strokeWidth={3} /> 更好的說法
                  </p>
                  <p>「{b.good}」</p>
                </div>
              </div>
